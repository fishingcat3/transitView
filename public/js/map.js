...
const mapURLs = {
    ...
    "Voyager (labels under)":
        "https://{s}.basemaps.cartocdn.com/rastertiles/voyager_labels_under",
...
L.tileLayer(
    `${
        mapURLs[settings.get("map-style") || "Voyager (labels under)"]
    }/{z}/{x}/{y}${L.Browser.retina ? "@2x" : ""}.png`,
    {
        minZoom: 6,
        maxZoom: 21,
        attribution:
            '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>, &copy; <a href="https://carto.com/attributions">CARTO</a>, &copy; <a href="https://opendata.transport.nsw.gov.au/datalicence">TfNSW</a>',
    }
).addTo(map);
...
async function vehicleToggleButtons() {
    ...
    vehicleTypes.forEach((vehicle) => {
        ...
        img.style["opacity"] = mapFilters.get("vehicle").includes(vehicle)
            ? 1
            : 0.4;
    ...
    toggleButtons.forEach((toggleButton) => {
        function handleToggleVehicleView(event) {
            ...
            toggleButton.addEventListener("transitionend", () => {
                ...
                vehicleUpdate(
                    `view_toggle_${opacity == 0.4 ? "hide" : "show"}`
                );
            });
        }
...
async function checkboxHandling() {
    ...
    checkboxes.forEach((checkbox) => {
        ...
        if (id == "checkbox-display_station_icons") {
            ...
            initialState =
                mapFilters.get("display_unscheduled_vehicles") == "true";
        }
        if (initialState) {
...
async function backCancelButtons() {
    ...
    getElement("back-side-bar").addEventListener(
        "click",
        handleBackButtonSideBar
    );
    getElement("back-side-bar").addEventListener(
        "keydown",
        handleBackButtonSideBar
    ...
    getElement("close-side-bar").addEventListener(
        "click",
        handleCancelButtonSideBar
    );
    getElement("close-side-bar").addEventListener(
        "keydown",
        handleCancelButtonSideBar
...
function handleDropdowns(reposition) {
    const specAttDropdown = new Dropdown(
        "specialattributes",
        "size-2",
        [
            "Articulated Bus",
    ...
    const vehicleTypeDropdown = new Dropdown(
        "vehicletype",
        "size-1",
        [
            "Waratah A",
    ...
    if (reposition) {
        function repositionDropdowns() {
            updateDropdown(
                vehicleTypeDropdown,
                mapFilters.get("special-attributes")
            );
            updateDropdown(
                specAttDropdown,
                mapFilters
                    .get("train-set-type")
                    .map((set) =>
                        vehicleTypeDropdown.options.find((fullName) =>
                            fullName.endsWith(set)
                        )
                    )
            );
        }
    ...
    getElement(
        "dropdown-current-specialattributes"
    ).innerHTML = `Bus Attribute Filters (${
        mapFilters.get("special-attributes").length
    }/${mapFilters.getOptions("special-attributes").length})`;
    ...
    specAttDropdown.options.forEach((option) => {
        specAttDropdownListeners[
            `dropdown-${specAttDropdown.id}-${specAttDropdown.options.indexOf(
                option
            )}`
        ] = (event) => {
            let filters = mapFilters.get("special-attributes");
            if (filters.includes(option)) {
                filters = filters.filter((x) => x != option);
            } else {
                filters.push(option);
            }
    ...
    getElement(
        "dropdown-current-vehicletype"
    ).innerHTML = `Train Set Type Filters (${
        mapFilters.get("train-set-type").length
    }/${mapFilters.getOptions("train-set-type").length})`;
    const vehicleTypeDropdownListeners = {
        "dropdown-vehicletype": (event) => {
            const filtersFullName = mapFilters
                .get("train-set-type")
                .map((set) =>
                    vehicleTypeDropdown.options.find((fullName) =>
                        fullName.endsWith(set)
    ...
    vehicleTypeDropdown.options.forEach((option) => {
        vehicleTypeDropdownListeners[
            `dropdown-${
                vehicleTypeDropdown.id
            }-${vehicleTypeDropdown.options.indexOf(option)}`
        ] = (event) => {
            ...
            vehicleUpdate();
            const filtersFullName = filters.map((set) =>
                vehicleTypeDropdown.options.find((fullName) =>
                    fullName.endsWith(set)
                )
            );
    ...
    window.addEventListener("resize", (event) => {
        if (
            [oldWidth <= 992, event.currentTarget.innerWidth <= 992].filter(
                (x) => !!x
            ).length == 1
        ) {
        ...
        const size =
            (direction == "width"
                ...
                : document.body.clientHeight -
                  (event.clientY || event.changedTouches[0].clientY)) - 28;
...
async function handleInstance() {
    ...
    params.forEach((value, key) => {
        if (key != "instanceId") params.delete(key);
    });
    history.replaceState(
        {},
        "",
        `${new URL(window.location.href).pathname}?${params.toString()}`
    );
}
...
async function loadVehicleInstance(params, instanceId) {
    ...
    if (vehicle?.TripInstance?.path && vehicle?.TripInstance?.shapeId) {
        instanceLayerGroup.clearLayers();
        const shapes = await fetchRouteShapes(
            vehicle.TripInstance?.path,
            `${vehicle.TripInstance?.shapeId};${vehicle.TripInstance?.route?.id}`,
            false
        );
        for (const shape of Object.values(shapes)) {
            const color = ((x) => (x.startsWith("#") ? x.slice(1) : x))(
                shape.color ||
                    vehicle?.TripInstance?.route?.color ||
                    (vehicle.VehicleInstance?.type == "bus"
                        ? "00B5EF"
                        : "8F8F8F")
            );
...
async function updateVehicleInstance(params, instanceId, vehicle, pan) {
    ...
    if (
        !vehicle ||
        vehicle == {} ||
        !vehicle.TripInstance ||
        !vehicle.VehicleInstance ||
        vehicle.error
    ) {
        return await loadPartial("side-bar", "../html/partials/error.html");
    }
    ...
    vehicleRouteIcon.style["background-color"] =
        vehicle.TripInstance?.route?.color;
    vehicleRouteIcon.style["color"] = vehicle.TripInstance?.route?.textColor;
    vehicleRouteIcon.style["font-weight"] = [0, 800, 600, 400, 350][
        vehicle.TripInstance?.route?.shortName?.length || 1
    ];
    vehicleRouteIcon.innerHTML = vehicle.TripInstance?.route?.shortName || "?";

    const vehicleTitle = getElement("vehicle-instance-title");
    vehicleTitle.innerHTML = ((x) =>
        x == "" ? vehicle.TripInstance?.route?.longName || "Unknown" : x)(
        (
            Object.values(vehicle.TripInstance?.headSign || {}) || ["Unknown"]
        ).join(" via ")
    );
    ...
    const model = ((model, scheduledSet, type) => {
        if (type == "bus") {
            return [
                "Model",
                `${((x) => (x == "" ? "Unknown" : String(x)))(
                    String(Object.values(model).join(" "))
                        .replaceAll("Unknown", "")
                        .trim()
                )}`,
            ];
            ...
            return [
                `${scheduledSet ? "Scheduled" : "Model"}`,
                `${((x, y) =>
                    `${x?.vehicle_category_name || y.name} (${
                        x?.vehicle_category_id || y.letter
                    })`)(scheduledSet, model)}`,
            ];
        } else {
        ...
        vehicle?.VehicleInstance?.model,
        vehicle.TripInstance?.scheduledSet,
        vehicle?.VehicleInstance?.type
    );
    ...
    new Table("vehicle-information-table", {
        ...
        "Direction bound":
            typeof vehicle.TripInstance?.directionBound == "number"
                ? `${
                      vehicle.TripInstance.directionBound == 0
                          ? "Outbound"
                          : "Inbound"
                  } (${vehicle.TripInstance.directionBound})`
                : null,
        "Schedule relationship": vehicle.TripInstance?.scheduleRelationship,
        "Shape ID": vehicle.TripInstance?.shapeId,
        "Last updated": vehicle.TripInstance?.time
            ? `<time>${formatDate(vehicle?.TripInstance?.time * 1000)}</time>`
            : null,

        Route: "SECTION_HEADER",
        ID: vehicle.TripInstance?.route?.id,
        Agency: vehicle.TripInstance?.route?.agency
            ? `${vehicle.TripInstance.route.agency.name} (${vehicle.TripInstance.route.agency.id})`
            : null,
        Colour: vehicle.TripInstance?.route?.color
            ? `<div class="color-box" style="background-color: ${
                  vehicle?.TripInstance?.route?.color
              };  color: ${
                  hexToLuminance(vehicle?.TripInstance?.route?.color) >= 0.6
                      ? "#000000"
                      : "#FFFFFF"
              }">${vehicle?.TripInstance?.route?.color}</div>`
            : null,
        "Text colour": vehicle.TripInstance?.route?.textColor
            ? `<div class="color-box" style="background-color: ${
                  vehicle?.TripInstance?.route?.textColor
              }; color: ${
                  hexToLuminance(vehicle?.TripInstance?.route?.textColor) >= 0.6
                      ? "#000000"
                      : "#FFFFFF"
              }">${vehicle?.TripInstance?.route?.textColor}</div>`
            : null,
        ...
        "Last position": Object.entries(
            vehicle.VehicleInstance?.lastPosition?.coordinates
        )
            .map(
                (x) =>
                    `${x[0] == "lat" ? "Latitude" : "Longitude"}: ${parseFloat(
                        Math.abs(x[1]).toFixed(7)
                    )}°${
                        x[0] == "lat"
                            ? x[1] > 0
                                ? "N"
            ...
            .join("\n"),
        "Last bearing": vehicle.VehicleInstance?.lastPosition?.bearing
            ? `${bearingToCardinal(
                  vehicle.VehicleInstance.lastPosition.bearing
              )} (${parseFloat(
                  vehicle.VehicleInstance.lastPosition.bearing.toFixed(2)
              )}°)`
            : null,
        "Last location": vehicle.VehicleInstance?.lastPosition?.location,
        "Last speed":
            typeof vehicle.VehicleInstance?.lastPosition?.speed == "number"
                ? `${parseFloat(
                      vehicle.VehicleInstance.lastPosition.speed.toFixed(3)
                  )} km/h`
                : null,
        "Last updated": vehicle.VehicleInstance?.lastPosition?.time
            ? `<time>${formatDate(
                  vehicle?.VehicleInstance?.lastPosition?.time * 1000
              )}</time>`
            : null,
        [model[0]]: vehicle.VehicleInstance?.model ? model[1] : null,
        Status: vehicle.VehicleInstance?.status
            ? vehicle?.VehicleInstance?.status.replaceAll("_", " ")
            : null,

        "Bus filter attributes":
            vehicle.VehicleInstance?.specialAttributes?.length > 0
                ? vehicle?.VehicleInstance?.specialAttributes.join("\n")
                : null,

        Other: "SECTION_HEADER",
        "Feed path": vehicle.TripInstance?.path
            ? vehicle.TripInstance.path
                  ?.replace("regionbuses", "regionbuses/")
                  ?.replace("lightrail", "lightrail/")
                  ?.replace("ferries", "ferries/")
            : null,
    });

    const timetableTable = vehicle.StoppingPattern?.timetable?.map(
        (stopEvent) => {
            return [
                `${stopEvent.stop?.name || `ID: ${stopEvent.stopId}`}${
                    stopEvent.timepoint == true ? "\n(Timepoint)" : ""
                }`,
                `ARR: <time>${stopEvent.arr}</time>\nDEP: <time>${stopEvent.dep}</time>`,
            ];
        }
    );

    new Table(
        "stopping-information-table",
        Object.fromEntries(
            [["Static Timetable", "SECTION_HEADER"]].concat(
                timetableTable.length == 0
                    ? ["Unknown", "Unknown"]
                    : timetableTable
            )
        )
    ...
    if (pan) {
        map.panTo(vehicle.VehicleInstance?.lastPosition?.coordinates, {
    ...
    setTimeout(() => {
        updateVehicleInstance(params, instanceId, null, false);
    }, 10000);
}
...
async function updateStopInstance(params, instanceId, stop, pan) {
    ...
    stopInstanceTypeIcon.src = `../icons/nsw/${
        stop.name.includes("Coach Stop") ? "coach" : stop.type
    }.png`;
    stopInstanceTypeIcon.alt = `${
        stop.name.includes("Coach Stop") ? "coach" : stop.type
    } icon`;

    getElement("stop-instance-title").innerHTML =
        stop.subName?.headName || stop.name;
    ...
    if (stop.subName?.subtitle) {
        ...
        subtitle.innerHTML = `${(stop?.subName?.delim || "")
            .replace(",", "")
            .trim()}${stop?.subName?.delim ? " " : ""}${stop.subName.subtitle}`;
    ...
    setTimeout(() => {
        updateStopInstance(params, instanceId, null, false);
    }, 10000);
}
...
async function updateLastUpdated() {
    if (!getElement("last-updated")) {
        return (lastUpdatedActive = false);
    }
    lastUpdatedActive = true;
    getElement("last-updated").innerHTML = `${sToHMS(
        (Date.now() - lastUpdated) / 1000
    )} ago`;
    getElement("last-updated").innerHTML += `${
...
function vehicleHTMLmarker(
  ...
  <div class="vehicle length-${
      label?.length || "3"
        ...
        scheduleDeviation
            ? ` box-shadow: ${delayColors[scheduleDeviation]} 0px 0px 5px 0.5px;`
            : ""
    }">
    ${
        bearing
            ? `<div class="vehicle-arrow" style="${
                  bearing - 90 < -320 || bearing - 90 < 70
                      ? "top: 2px; right: 0.5px; position:relative;"
                      : ""
...
function getDisplayVehicles() {
    return vehicleTypes.filter(
        (vehicleType) =>
            getElement(`toggle-view-${vehicleType}`) &&
            getComputedStyle(getElement(`toggle-view-${vehicleType}`))
                .opacity == 1
    );
}
...
async function fetchVehicles(vehiclesToFetch) {
    ...
    const response = await fetch(
        `${url.origin}/api/nsw/vehicles?type=${vehiclesToFetch.join(
            ","
        )}${boundsQuery(map.getBounds())} `,
        {
            method: "GET",
            headers: {
...
async function fetchStops(stopsToFetch) {
    ...
    const response = await fetch(
        `${url.origin}/api/nsw/stops?type=${stopsToFetch.join(
            ","
        )}${boundsQuery(map.getBounds())} `,
        {
            method: "GET",
...
async function fetchRouteShapes(path, routeShapes, cache) {
    ...
    const response = await fetch(
        `${
            url.origin
        }/api/nsw/route_shape?path=${path}&routes=${routeShapes.join(",")}${
            cache ? `&cache=true` : ""
        } `,
        {
            method: "GET",
...
async function fetchInstance(instanceId) {
    const response = await fetch(
        `${url.origin}/api/nsw/instance?instanceId=${instanceId}`,
        {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Cache-Control": `max-age=${
                    instanceId.split("/")[0] == "stop" ? "86400" : "4"
                } `,
            },
        }
    );
...
function updateMarkerTooltip(event) {
    currentMarkerHover =
        document.body.clientWidth <= 768 ? undefined : currentMarkerHover;
    ...
    if (currentMarkerHover) {
        ...
        const markerPosition = ((mapPosition, markerInMapPosition) => {
            (mapPosition = mapPosition || { x: 0, y: 0 }),
                (markerInMapPosition = markerInMapPosition || { x: 0, y: 0 });
            return {
                x: markerInMapPosition.x + mapPosition.x,
                y: markerInMapPosition.y + mapPosition.y,
            };
        ...
        if (instanceIdSplit[0] == "vehicle") {
            const vehicle =
                vehicles[instanceIdSplit[1]].find(
                    (vehicle) =>
                        vehicle.VehicleInstance.instanceId == currentMarkerHover
                ) || {};

            const title = document.createElement("div");
            const routeShortName =
                vehicle?.TripInstance?.route?.shortName || "Unknown";
            const tripHeadSign = Object.values(
                vehicle?.TripInstance?.headSign || { headline: "Unknown" }
            ).join(" via ");
            title.innerHTML = `<span class="text-bold"> ${routeShortName}</span> ${
                ["Empty Train", "Non Timetabled", "Non Revenue"].some((x) =>
                    tripHeadSign.includes(x)
                )
                    ? " "
                    : " to "
            }${tripHeadSign != "" ? tripHeadSign : "Unknown"}
      ${
          vehicle?.VehicleInstance?.specialAttributes?.includes("Christmas Bus")
            ...
            title.style["background-color"] =
                vehicle?.TripInstance?.route?.color || "#000000";
            title.style["color"] =
                vehicle?.TripInstance?.route?.textColor || "#FFFFFF";
            ...
            ((model, scheduledSet, type) => {
                if (type == "bus") {
                    subtext.innerHTML = `Vehicle Model: ${((x) =>
                        x == "" ? "Unknown" : String(x))(
                        String(Object.values(model).join(" "))
                            .replaceAll("Unknown", "")
                            .trim()
                    ...
                    subtext.innerHTML = `${
                        scheduledSet ? "Schedule Vehicle: " : "Vehicle Model: "
                    }${((x, y) =>
                        `${x?.vehicle_category_name || y.name} (${
                            x?.vehicle_category_id || y.letter
                        })`)(scheduledSet, model)} `;
            ...
            subtext2.innerHTML += `Position last updated: ${sToHMS(
                (Date.now() -
                    vehicle.VehicleInstance?.lastPosition?.time * 1000) /
                    1000
            )} ago`;
            ...
            if (
                markerPosition.x + toolTip.offsetWidth + 40 <
                document.body.clientWidth
            ) {
                // right
                toolTip.style.left = `${markerPosition.x + 40}px`;
                toolTip.style.top = `${
                    markerPosition.y - toolTip.offsetHeight / 2 + 2.5
                }px`;
            } else {
                // left
                toolTip.style.left = `${
                    markerPosition.x - 15 - toolTip.offsetWidth
                }px`;
                toolTip.style.top = `${
                    markerPosition.y - toolTip.offsetHeight / 2 + 2.5
                }px`;
            ...
            const stop =
                stops[instanceIdSplit[1]].find(
                    (stop) => stop.instanceId == currentMarkerHover
                ) || {};
            ...
            headName.innerHTML = stop?.subName?.headName
                ? stop.subName.headName
                : stop.name;
            ...
            if (stop.subName?.subtitle) {
                ...
                subtitle.innerHTML = `${(stop?.subName?.delim || "")
                    .replace(",", "")
                    .trim()}${stop?.subName?.delim ? " " : ""}${
                    stop?.subName?.subtitle
                } `;
            ...
            if (
                ...
                toolTip.style.top = `${
                    markerPosition.y - toolTip.offsetHeight / 2
                }px`;
            } else {
                // left
                toolTip.style.left = `${
                    markerPosition.x - 14 - toolTip.offsetWidth
                }px`;
                toolTip.style.top = `${
                    markerPosition.y - toolTip.offsetHeight / 2
                }px`;
...
function markerHoverStart(event) {
    currentMarkerHover =
        event?.sourceTarget?.options?.icon?.options?.instanceId;
    if (!currentMarkerHover && !mapMoving) {
        return;
    }
...
async function markerClick(event) {
    ...
    map.panTo(event.sourceTarget._latlng, {
    ...
    history.pushState(
        {},
        "",
        `?instanceId=${encodeURIComponent(markerClicked)} `
    );
    await handleInstance();
}
...
async function drawRoutes(path, routeShapesIds) {
    ...
    for (const routeShape of Object.keys(routeShapes)) {
        ...
        const polyline = L.polyline(filterPointsMinDistance(latLngs, 60), {
            color: `#${routeShapes[routeShape].color} `,
            weight: 2.5,
            offset: 2,
            smoothFactor: 2.5,
        });
...
function updateTolerance() {
    ...
    for (const [key, value] of Object.entries(
        Object.assign({}, ...Object.values(routePolylines))
    )) {
        if (!key || !value) {
            return;
        }
...
async function mapUpdate(updateType) {
    ...
    if (!map.hasLayer(stopLayerGroup)) {
        return;
    }
    const displayStops = Object.entries(stopTypesMinZoom)
        .filter(([zoom]) => Number(zoom) <= map._zoom)
        .flatMap(([, stopTypes]) => stopTypes);
        ...
        stops[stopType].forEach((stop) => {
            ...
            if (!markers[instanceId]) {
                const stopMarker = marker(
                    ...
                    "icon",
                    `nsw/${
                        stop.name.includes("Coach Stop") ? "coach" : stopType
                    }.png`,
                    "",
                    instanceId,
                    0
                ...
                stopMarker
                    .on("mouseover", markerHoverStart)
                    .on("mouseout", markerHoverEnd)
    ...
    Object.keys(markers)
        .filter((marker) => marker.startsWith(`stop`))
        .forEach((instanceId) => {
            if (
                !stops[instanceId.split("/")[1]]?.some(
                    (stop) => stop.instanceId == instanceId
                )
            ) {
...
function vehicleOpacity(auto) {
    const params = new URLSearchParams(window.location.search);
    const instanceDisplayed =
        auto ||
        !!((x) =>
            x &&
            x.has("instanceId") &&
            x.get("instanceId").split("/")[0] == "vehicle")(params);
    const instanceId = instanceDisplayed ? params.get("instanceId") : null;
    Object.values(vehicleLayerGroup._layers).forEach((x) => {
        x._icon.style["opacity"] = instanceDisplayed
            ? x.options.icon.options.instanceId == instanceId
                ? 1
                : 0.5
            : 1;
...
async function vehicleUpdate(updateType) {
    const displayVehicles = vehicleTypes.filter((vehicleType) =>
        mapFilters.get("vehicle").includes(vehicleType)
    );
    if (updateType == "view_toggle_hide") {
        vehicles = Object.fromEntries(
            Object.entries(vehicles).filter(([key]) =>
                displayVehicles.includes(key)
            )
        );
        ...
        for (const vehicleType of Object.keys(vehicles)) {
            vehicles[vehicleType].filter((vehicle) => {
                ...
                const inBounds =
                    lat &&
                    lat > newBounds._southWest.lat &&
                    lat < newBounds._northEast.lat &&
                    lng &&
                    lng > newBounds._southWest.lng &&
                    lng < newBounds._northEast.lng;
    ...
    Object.keys(markers)
        .filter((marker) => marker.startsWith(`vehicle`))
        .filter(
            (instanceId) =>
                !displayVehicles.some(
                    (vehicleType) => instanceId.split("/")[1] == vehicleType
                )
        )
        .forEach((instanceId) => {
    ...
    for (const vehicleType of Object.keys(vehicles)) {
        vehicles[vehicleType].forEach((vehicle) => {
            ...
            if (!position?.coordinates?.lat) {
                return;
            }

            const vTi = vehicle?.TripInstance;
            const instanceId = vehicle.VehicleInstance?.instanceId;
            const vehicleIcon = vehicleHTMLmarker(
                vTi?.route?.shortName,
                vTi?.serviceDeviation,
                vTi?.route?.color,
                vTi?.route?.textColor,
                position?.bearing
            );
            ...
            const isHidden =
                (mapFilters.get("display_unscheduled_vehicles") == "false" &&
                    vehicle?.TripInstance?.scheduleRelationship ==
                        "UNSCHEDULED") ||
                (mapFilters.get("special-attributes").length > 0 &&
                    !vehicle?.VehicleInstance?.specialAttributes.some(
                        (specialAttribute) =>
                            mapFilters
                                .get("special-attributes")
                                .includes(specialAttribute)
                    )) ||
                (mapFilters.get("train-set-type").length > 0 &&
                    !mapFilters
                        .get("train-set-type")
                        .includes(vehicle.VehicleInstance?.model?.letter));
            ...
            if (markers[instanceId]) {
                ...
                if (
                    ...
                    markers[instanceId].slideTo(position?.coordinates, {
                        duration: 500,
                        keepAtCenter: false,
                    });
                }
    ...
    Object.keys(markers)
        ...
        .forEach((instanceId) => {
            if (
                !vehicles[instanceId.split("/")[1]]?.some(
                    (vehicle) =>
                        vehicle.VehicleInstance?.instanceId == instanceId
                )
            ) {
                deleteVehicleMarker(instanceId);
            }
...
window.addEventListener("unhandledrejection", (error) => {
    console.warn(`Unhandled rejection: ${error.reason}`);
});
...
map.addEventListener("moveend", () => {
    ...
    const zoomType =
        newBounds._southWest.lat > oldBounds._southWest.lat &&
