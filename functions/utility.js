...
export const getBearing = function (coord1, coord2) {
    ...
    const φ1 = rad(coord1.lat), φ2 = rad(coord2.lat), Δλ = rad(coord2.lng - coord1.lng);
    const x = Math.sin(Δλ) * Math.cos(φ2), y = Math.cos(φ1) * Math.sin(φ2) - Math.sin(φ1) * Math.cos(φ2) * Math.cos(Δλ);
...
export const directoryTree = function (dir, collapse, prefix = "") {
    ...
    files.forEach((file, index) => {
        ...
        if (collapse.some((pattern) => pattern.startsWith('%') ? filePath.includes(pattern.slice(1)) : path.resolve(filePath) == path.resolve(pattern))) {
            if (fs.statSync(filePath).isDirectory()) {
                let count = [0, 0]
                fs.readdirSync(filePath).forEach(file => { count[fs.statSync(path.join(filePath, file)).isDirectory() ? 1 : 0]++; });
                result.push(`${prefix}${last ? '└── ' : '├── '}${file} <${count[0]} file${count[0] != 1 ? `s` : ``}, ${count[1]} director${count[1] != 1 ? `ies` : `y`}>`);
            };
            return;
        };
        result.push(`${prefix}${last ? '└── ' : '├── '}${file}`);
        if (fs.statSync(filePath).isDirectory()) {
            const subdirectory = directoryTree(filePath, collapse, `${prefix}${last ? '    ' : '│   '}`);
            if (subdirectory != "") { result.push(subdirectory); };
        };
    });
...
export class Timer {
    ...
    start() {
        ...
        return log.greenB(`FINISH ${this.name} ${divider(Date.now() - this.startTime)}ms`);
    };
    get timeSinceStart() {
