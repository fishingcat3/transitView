...
export class Vehicle {
    static TripInstance = class {
        constructor(
            ...
            const schRel = Enum(
                trip.trip?.scheduleRelationship ||
            ...
            if (!(tripInfo || routeInfo || agencyInfo) && type == "train") {
                const unscheduled =
                    !tripInfo || this.id.startsWith("NonTimetabled");
            ...
            if (stopTimes?.timetable?.length > 0) {
                altHeadSign = stopTimes?.timetable?.find(
                    (stopTime) =>
                        stopTime.stop_id ==
                        (trip?.stopTimeUpdate?.length > 0
                            ? trip?.stopTimeUpdate[0].stopId
                            : "")
                )?.stop_headsign;
            }
            headSign =
                altHeadSign && altHeadSign != ""
                    ? altHeadSign
                    : tripInfo?.trip_headsign;
            headSign = (
                (headSign?.split(" via ")?.length == 1
                    ? headSign?.split(" Via ")
                    : headSign?.split(" via ")) || []
            ).map((x) => x?.trim());
            this.headSign = {
            ...
            if (this.headSign.headline == "") {
                this.headSign.headline = "Non Revenue";
            }
            ...
            this.scheduledSet = ["train", "train_link"].includes(type)
                ? databaseFind(
                      tripInfo?.vehicle_category_id,
                      "vehicle_category_id",
                      `${path}_vehicle_categories`,
                      "*"
                  )
                : null;
            ...
            this.route = {
                ...
                shortName: ((x) =>
                    x == ""
                        ? routeInfo?.route_long_name == "Out Of Service"
                            ? "OS"
                            : idSplit[0]
                        : x)(routeInfo?.route_short_name),
            ...
            if (
                this.route.shortName == "" &&
                this.route.longName == "Non Revenue"
            ) {
    ...
    static VehicleInstance = class {
        constructor(
            ...
            this.instanceId = `vehicle/${type}/${day()}/${
                this.tripId.startsWith("NonTimetabled")
                    ? this.tripId
                    : this.tripId.split(".")[0]
            }`;
            ...
            if (vehicle.vehicle) {
                ...
                this.model =
                    vehicle.vehicle[".transit_realtime.tfnswVehicleDescriptor"]
                ...
                if (type == "bus") {
                    ...
                    if (this.model.chassis == "B12BLEA") {
                        this.specialAttributes.push("Articulated Bus");
                    }
                    if (
                        ["ZK6131HG1", "D9RA", "BYDK9", "ELEMENT"].includes(
                            this.model.chassis
                        )
                    ) {
                        this.specialAttributes.push("Electric Bus");
                    }
                } else if (["train", "train_link"].includes(type)) {
                    this.model = trainVehicleTypes[this.model] || {
                        name: "Unknown",
                        letter: this.model,
                    };
                ...
                this.aircon =
                    vehicle.vehicle[".transit_realtime.tfnswVehicleDescriptor"]
                        ?.airConditioned || "unknown";
                this.wheelchair = vehicle.vehicle[
                ...
                if (specialAttributes && specialAttributes != 0) {
                    if ((specialAttributes % 16) / 8 >= 1) {
                        this.specialAttributes.push("Social Distancing");
                    }
                    if ((specialAttributes % 8) / 4 >= 1) {
                        this.specialAttributes.push("Christmas Bus");
                    }
                    if ((specialAttributes % 4) / 2 >= 1) {
                        this.specialAttributes.push("Wi-Fi");
                    }
                    if ((specialAttributes % 2) / 1 >= 1) {
                        this.specialAttributes.push("Special Livery");
                    }
                }
                if (type == "bus") {
                    const routeName = tripInstance?.route?.shortName || "";
                    if (
                        tripInstance?.route?.description == "Temporary Buses" ||
                        routeName[routeName.length - 2] == "T" ||
                        routeName == "535"
                    ) {
                        this.specialAttributes.push("Temporary Bus");
                    }
                    if (routeName.startsWith("M")) {
                        this.specialAttributes.push("Metro Bus");
                    }
                    if (routeName.startsWith("N")) {
                        this.specialAttributes.push("NightRide");
                    }
                    if (routeName.startsWith("SW")) {
                        this.specialAttributes.push("South West Link");
                    }
                    if (path == "buses") {
                    ...
                    if (tripInstance?.route?.description == "School Buses") {
                        this.specialAttributes.push("School Bus");
                    }
                    if (
                        routeName.includes("X") &&
                        tripInstance?.route?.description ==
                            "Sydney Buses Network"
                    ) {
                        this.specialAttributes.push("Express");
                    }
                }
            }
    ...
    static StoppingPattern = class {
        constructor(
            ...
            stopTimes.timetable = stopTimes.timetable
                ? stopTimes.timetable
                      ...
                      .map((stopEvent) => {
                          return {
                              ...
                              note: ((x) => !x || x == "")(stopEvent.stop_note)
                                  ? null
                                  : stopEvent.stop_note,
                          };
                      })
    ...
    constructor(
        ...
        if (
            vehiclePos?.vehicle?.vehicle[
                ".transit_realtime.tfnswVehicleDescriptor"
            ]?.performingPriorTrip &&
            path != "lightrailnewcastle"
        ) {
            return null;
        }
        const newTripUpdate = tripUpdate?.tripUpdate || {};
        const newVehiclePos = vehiclePos?.vehicle || {};
        this.TripInstance = new Vehicle.TripInstance(
            newTripUpdate,
            newVehiclePos,
            tripInfo,
            routeInfo,
        ...
        this.VehicleInstance = new Vehicle.VehicleInstance(
            newVehiclePos,
            null,
            null,
            null,
        ...
        this.StoppingPattern = new Vehicle.StoppingPattern(
            null,
            null,
            null,
            null,
