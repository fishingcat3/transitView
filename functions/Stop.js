...
export class StopInstance {
    constructor(stop, stopInfo, type) {
        ...
        if (type == "bus") {
            const delimSplit = delims.find((delim) => stopName.existsSplit(this.name, delim));
            this.subName = delimSplit ? stopName.subName(this.name, delimSplit) : {};
        ...
        this.parentStation = !!stop.parent_station ? stop.parent_station : undefined;
        this.wheelchair = !!stop.wheelchair_boarding;
        this.levelId = !!stop.level_id ? stop.level_id : undefined;
        this.platformNumber = !!stop.platform_code ? stop.platform_code : undefined;
        ...
        if (stopInfo) {
            this.stopInfo = {
                ...
                transit_modes: stopInfo.TRANSPORT_MODE.toLowerCase().split(",").map((x) => x.trim()),
