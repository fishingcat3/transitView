...
import { API_Keys, __dirname, setPendingGTFSUpdate, gtfsPaths, getLastGTFSUpdate, setLastGTFSUpdate } from '../server.js';
...
async function gtfsAPI({ version, path }, method) {
    return await fetch(`https://api.transport.nsw.gov.au/v${version}/gtfs/schedule/${path}`, {
        method: method, mode: "cors", headers: { accept: "application/octet-stream", authorization: `apikey ${API_Keys.NSW1}` }
    });
};
...
async function deleteGTFS(path) {
    ...
    fs.rmSync(`./cache/nsw/gtfs_${path}`, { recursive: true, force: true }, (err) => {
        if (err) { log.redB(`FAILED DELETE GTFS ${err.message}`); };
    });
    t.end();
};

export async function updateGTFS({ version, path }) {
    if (Date.now() - getLastGTFSUpdate(path) < 600000) { return; };
    const t = new Timer(`GTFS UPDATE ${path.padStart(34, " ")}`).start();
    version = version[0] || version;
    const preresponse = await gtfsAPI({ version, path }, "HEAD");
    const savedLastModified = JSON.parse(fs.readFileSync("./cache/nsw/last_updated.json", { encoding: "utf8", flag: "r" }) || "{}");
    ...
    if (lastModified == (savedLastModified[path] || undefined)) {
        ...
        return log.greenB(`FINISH GTFS ${path.padStart(40, " ")} UPDATED <${formatDate(lastModified)}> ${t.timeSinceStart}ms`);
    };
    ...
    try {
        ...
        await processTables(workingPath)
            .then(async () => {
                ...
                fs.writeFileSync("./cache/nsw/last_updated.json", JSON.stringify(savedLastModified, null, 4));
                t.end();
                setLastGTFSUpdate(path, Date.now());
